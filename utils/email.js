...
const verifiedUser = async (email, user, value) => {
	try {
		...
		const mailOptions = {
			...
			to: email,
			subject: "Organisation Account Verification",
			html: ` <h3>
            This mail, is for account verification... Please use the <a
            href="${url}/api/organisation/${user}/${value}"
...
const resetUserPassword = async (email, user, value) => {
	try {
		...
		const mailOptions = {
			...
			html: ` <h3>
            This mail, is for account verification... Please use the <a
            href="${url}/api/organisation/change/${user}/${value}"
            >Link to Finish</a> up your account creation 
        </h3>`,
		};
...
const verifiedClient = async (email, user, value) => {
	try {
		const accessToken = await oAuth.getAccessToken();
		const transporter = nodemailer.createTransport({
			service: "gmail",
		...
		const mailOptions = {
			...
			html: ` <h3>
            This mail, is for account verification... Please use the <a
            href="${url}/api/client/${user}/${value}"
            >Link to Finish</a> up your account creation 
        </h3>`,
		};
...
const verifiedSignClient = async (email, user, value) => {
	try {
		const accessToken = await oAuth.getAccessToken();
		const transporter = nodemailer.createTransport({
			service: "gmail",
...
const resetClientPassword = async (email, user, value) => {
	try {
		const accessToken = await oAuth.getAccessToken();
		const transporter = nodemailer.createTransport({
			service: "gmail",
		...
		const mailOptions = {
			...
			html: ` <h3>
            This mail, is for account verification... Please use the <a
            href="${url}/api/client/change/${user}/${value}"
            >Link to Finish</a> up your account creation 
        </h3>`,
		};
